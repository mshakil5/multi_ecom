...
<div class="container-fluid">
    <h2 class="position-relative text-uppercase text-center mb-4">
        <span class="bg-secondary pr-3">Register As Supplier</span>
    </h2>
                ...
                <form name="registerForm" id="registerForm" method="POST" action="{{ route('supplier.register') }}">
                    ...
                    <div class="control-group mb-3">
                        <input type="text" class="form-control @error('phone') is-invalid @enderror" id="phone" name="phone" placeholder="Your Phone"
                            value="{{ old('phone') }}" required="required" data-validation-required-message="Please enter your phone number" />
                        @error('phone')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                    </div>
